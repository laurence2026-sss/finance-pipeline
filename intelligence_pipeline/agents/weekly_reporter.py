...
→ 시그널 1개만 포착된 경우: ⚠️ 표기 후 "확인 필요" 명시

---

### 🔹 Part 2: 다음 주 자금 이동 예상 경로

> ⚠️ 이 파트는 예측입니다. 이번 주 데이터에서 포착된 시그널에만 근거하며,
> 실현 여부는 불확실합니다. 투자 판단의 참고 자료로만 활용하세요.

각 예측은 아래 형식으로 최대 3개까지만 제시합니다.

**① [예측 제목]**
- 신뢰도: ★★★☆☆ (시그널 강도 기준, 5점 만점)
- 근거: [이번 주 데이터에서 포착된 구체적 시그널, 수치 포함]
- 전제 조건: [이 예측이 맞으려면 필요한 조건]
- 수혜 예상 섹터/종목: [구체적 티커 포함]
...
> 자금 흐름 전체를 관통하는 가장 중요한 인사이트 3가지.
> 단순 뉴스 나열이 아닌, 여러 시그널이 수렴하는 구조적 의미를 서술합니다.

**① [포인트 제목]**
[배경·데이터·투자자 행동 변화를 연결한 설명, 3~4문장]
관련 티커: $XXX, $XXX
...
# RULES
- 모든 수치는 구체적으로 (예: "상승"이 아닌 "+1.8%", "+23bp")
- **데이터 보정**: US 10Y 국채 금리와 같은 지표는 백분율(%) 단위를 정확히 반영하세요. (예: 4.35%는 4.35로 표기)
- 데이터 공백 구간은 "해당 주간 데이터 없음"으로 처리, 내용 생성 금지
